...
export const ReportsView: React.FC<ReportsViewProps> = ({ onNavigateToPos }) => {
  ...
  const handleShareWeeklyWhatsApp = () => {
    const text = generateWeeklyReportWhatsApp(weeklySummary, tenant?.name || 'ComercioPro');
    openWhatsAppLink(tenant?.phone || '', text);
  };

  const handleShareMonthlyWhatsApp = () => {
    const text = generateMonthlyReportWhatsApp(monthlySummary, tenant?.name || 'ComercioPro');
    openWhatsAppLink(tenant?.phone || '', text);
  };

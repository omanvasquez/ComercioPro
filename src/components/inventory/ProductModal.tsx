...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const [category, setCategory] = useState<string>(
    productToEdit?.category || (categories.length > 0 ? categories[0].name : 'General')
  );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 text-xs">Categoría</label>
                <button
                  type="button"
                  onClick={() => setIsAddingNewCat(!isAddingNewCat)}
                  className="text-[11px] font-bold text-blue-600 hover:text-blue-700 hover:underline flex items-center space-x-0.5"
                >
                  <Plus className="w-3 h-3" />
                  <span>{isAddingNewCat ? 'Lista' : 'Nueva'}</span>
                </button>
              </div>
              ...
              {isAddingNewCat ? (
                <div className="flex items-center space-x-1.5">
                  <input
                    type="text"
                    placeholder="Ej. Tornillos, Papelería..."
                    value={newCatName}
                    onChange={(e) => setNewCatName(e.target.value)}
                    className="flex-1 px-3 py-2 rounded-xl border border-blue-500 text-xs font-semibold focus:outline-none bg-blue-50/40"
                    autoFocus
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <select
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 font-semibold bg-white text-xs"
                >
                  {categories.map((cat) => (
            ...
            <div className="space-y-1.5">
              <label className="font-bold text-slate-800 text-xs">Unidad de Medida</label>
              <select
                value={unit}
              ...
              >
                ...
                <option value="metro">Metro (m - Fraccional)</option>

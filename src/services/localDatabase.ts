import { Product, Customer, SaleTicket, CreditTransaction, InventoryWaste, Supplier, Tenant, Expense, Category } from '../types';

const INITIAL_TENANT: Tenant = {
  id: 'tenant_comercio_01',
  name: 'Comercio Express',
  ...
  createdAt: Date.now(),
  defaultWhatsAppMsg: 'Gracias por su compra en ComercioPro',
};
...
export const INITIAL_CATEGORIES: Category[] = [
  { id: 'cat_1', tenantId: 'tenant_comercio_01', name: 'General', color: '#2563eb', createdAt: Date.now() },
  { id: 'cat_2', tenantId: 'tenant_comercio_01', name: 'Víveres y Alimentos', color: '#10b981', createdAt: Date.now() },
  { id: 'cat_3', tenantId: 'tenant_comercio_01', name: 'Bebidas', color: '#06b6d4', createdAt: Date.now() },
  { id: 'cat_4', tenantId: 'tenant_comercio_01', name: 'Ferretería', color: '#f59e0b', createdAt: Date.now() },
  { id: 'cat_5', tenantId: 'tenant_comercio_01', name: 'Papelería', color: '#8b5cf6', createdAt: Date.now() },
  { id: 'cat_6', tenantId: 'tenant_comercio_01', name: 'Repuestos', color: '#ef4444', createdAt: Date.now() },
  { id: 'cat_7', tenantId: 'tenant_comercio_01', name: 'Limpieza', color: '#14b8a6', createdAt: Date.now() },
  { id: 'cat_8', tenantId: 'tenant_comercio_01', name: 'Otros', color: '#64748b', createdAt: Date.now() },
];

export function getLocalData<T>(key: string, defaultData: T): T {
  try {
    const saved = localStorage.getItem(`comerciopro_${key}`);
...
export function saveLocalData<T>(key: string, data: T): void {
  try {
    localStorage.setItem(`comerciopro_${key}`, JSON.stringify(data));
  } catch (e) {
...
export const dbInit = {
  ...
  getCategories: (): Category[] => getLocalData<Category[]>('categories', INITIAL_CATEGORIES),
  saveCategories: (c: Category[]) => saveLocalData('categories', c),
